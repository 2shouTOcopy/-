...
- `script_runtime_test.cpp` already covers:
  ...
  - bool/int/float array marshaling
  - `init()` runtime failure
  - output setter type mismatch handling
  - numeric values rejected by string setters
  - `run()` runtime error reporting
  - missing subscribed input reporting
  - timeout and Lua memory-limit error reporting
  - sparse int-array rejection at runtime
  - non-table and mixed-type array setter rejection
  - `cleanup()` failure still allowing runtime teardown
- `script.cpp` now requires real Lua strings for string setters and validates
  full array tables before publishing any output values.
- Host API definitions are centralized in `script_host_api.hpp/.cpp`:
  - static analysis and runtime Lua binding both consume this registry
  - each API declares name, API kind, value type, argument count, and lifecycle
    mask
  - topology getters, output setters, context getters, and reserved APIs are
    distinct kinds
- Topology getter implementation path:
  - `GetBoolValue()` and similar APIs are registry entries with kind
    `kScriptHostApiTopologyGetter`
  - `CScriptModule::InstallHostApis()` binds non-reserved registry entries into
    the Lua sandbox
  - all bound APIs enter `luaHostApiBridge()`
  - topology getters are dispatched to `CScriptModule::LuaGetValue()`
  - `LuaGetValue()` checks parsed subscription metadata, calls
    `ReadSubscriptionValue()`, appends debug input text, then pushes a typed Lua
    result
  - bool reads use the existing int dynamic input function
    `VM_M_GetInt_Dynamic()` and convert zero/nonzero to Lua boolean
- `GetGlobalCommunicationData()` is registered as a zero-argument context
  getter. It does not create subscription/publish metadata and is backed at
  runtime by a weak C provider:
  `Script_GetGlobalCommunicationData(char *buffer, int bufferSize)`.
...
## What To Continue
- Extend runtime tests only if new edge cases are identified beyond the current
  scalar/array/error/resource-limit coverage.
- Add future Lua host APIs through `script_host_api.hpp/.cpp` instead of adding
  separate hardcoded lists in parser and runtime code.
...
## Known Gaps
- `make -C source/algos/modules/script` is still blocked by repository-local
  build/include issues unrelated to the new logic:
  - existing `ScFrame.h` `-Werror,-Wreturn-type` failures
  - missing external `mvb_types.h` included from `mvb_types_ex.h`
...
- Remaining runtime coverage is no longer a known blocker; add more only for

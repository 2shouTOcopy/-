...
- The baseline lifecycle ABI, parser/analyzer path, runtime hosting path, and
  shipped template/config cleanup have now landed in code.
- Current remaining gaps are concentrated in:
  - broader runtime/integration test coverage
  - optional upper-layer regression coverage for the comif/adapter query path
  - native in-repo module build verification beyond the host-side test harness
- This document defines the target behavior that the remaining verification work
  must preserve.

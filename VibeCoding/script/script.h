...
struct ScriptHostValue {
  ...
  std::vector<int> bool_array_value;
  std::vector<int> int_array_value;
  std::vector<float> float_array_value;
...
public:
  ...
  int LuaGetValue(lua_State *L, mvsc::script::ScriptValueType type, int moduleNo,
                  const char *paramName);
  int LuaSetValue(lua_State *L, mvsc::script::ScriptValueType type,
                  const char *outputName, int valueIndex);
  int LuaGetContextValue(lua_State *L,
                         const mvsc::script::ScriptHostApiDef &api);
...
private:
  ...
  int BuildDynamicParamsFromAnalysis();
  ...
  int ReadSubscriptionValue(void *hInput, int index, ScriptHostValue *pValue);
  int ExecuteCompiledScript(void *hInput, void *hOutput);
  int CreateLuaRuntime(const std::string &script_text);
  void DestroyLuaRuntime();
  int ExecuteLifecycleFunction(const char *name, bool fail_on_missing,
                               std::string *pErrorMessage);
  int InstallHostApis(lua_State *L, int env_index);
  int FindSubscriptionIndex(int moduleNo, const char *paramName,
                            mvsc::script::ScriptValueType type) const;
  const mvsc::script::ScriptOutputDef *FindOutputDef(const char *outputName) const;
  void AppendDebugValue(const char *name, const std::string &value, char *buffer,
                        size_t buffer_size) const;
  void SyncRunMode(void *hInput);
  std::string DebugValueToString(const ScriptHostValue &value) const;

...
using mvsc::script::ScriptHostApiDef;
...
using mvsc::script::GetScriptHostApis;
using mvsc::script::ToTypeName;

extern "C" int Script_GetGlobalCommunicationData(char *buffer, int bufferSize)
#if defined(__GNUC__)
    __attribute__((weak))
#endif
    ;
...
bool getDenseArrayLength(lua_State *L, int table_index, lua_Integer *length) {
  if (L == NULL || length == NULL) {
    return false;
  }

  const int abs_table_index = lua_absindex(L, table_index);
  lua_Integer max_index = 0;
  lua_Integer element_count = 0;
...
const ScriptHostApiDef *scriptApiFromLua(lua_State *L) {
  return static_cast<const ScriptHostApiDef *>(
      lua_touserdata(L, lua_upvalueindex(2)));
}

int luaHostApiBridge(lua_State *L) {
  CScriptModule *module = scriptModuleFromLua(L);
  const ScriptHostApiDef *api = scriptApiFromLua(L);
  if (module == NULL) {
    return luaL_error(L, "script module context is unavailable");
  }
  if (api == NULL) {
    return luaL_error(L, "script host API context is unavailable");
  }

  switch (api->kind) {
  case mvsc::script::kScriptHostApiTopologyGetter: {
    const int module_no = static_cast<int>(luaL_checkinteger(L, 1));
    const char *param_name = luaL_checkstring(L, 2);
    return module->LuaGetValue(L, api->value_type, module_no, param_name);
  }
  case mvsc::script::kScriptHostApiOutputSetter: {
    const char *output_name = luaL_checkstring(L, 1);
    return module->LuaSetValue(L, api->value_type, output_name, 2);
  }
  case mvsc::script::kScriptHostApiContextGetter:
    return module->LuaGetContextValue(L, *api);
  default:
    return luaL_error(L, "script host API is not implemented");
  }
}
...
int CScriptModule::InstallHostApis(lua_State *L, int env_index) {
  ...
  const ScriptHostApiDef *apis = GetScriptHostApis(&api_count);
  for (std::size_t idx = 0; idx < api_count; ++idx) {
    if (apis[idx].kind == mvsc::script::kScriptHostApiReserved) {
      continue;
    }
    lua_pushlightuserdata(L, this);
    lua_pushlightuserdata(L, const_cast<ScriptHostApiDef *>(&apis[idx]));
    lua_pushcclosure(L, luaHostApiBridge, 2);
    lua_setfield(L, env_index, apis[idx].name);
  }
  return IMVS_EC_OK;
}
...
int CScriptModule::LuaGetContextValue(lua_State *L, const ScriptHostApiDef &api) {
  if (strcmp(api.name, "GetGlobalCommunicationData") == 0) {
    if (Script_GetGlobalCommunicationData == NULL) {
      return luaL_error(L, "global communication data provider is unavailable");
    }

    char buffer[MAX_RESLUT_STRING_LEN] = {0};
    const int nErrCode =
        Script_GetGlobalCommunicationData(buffer, sizeof(buffer));
    if (nErrCode != IMVS_EC_OK) {
      return luaL_error(L, "failed to get global communication data");
    }
    lua_pushstring(L, buffer);
    return 1;
  }

  return luaL_error(L, "unsupported context getter");
}
...
int CScriptModule::LuaSetValue(lua_State *L, ScriptValueType type,
  ...
  case mvsc::script::kScriptValueFloat:
    ...
    break;
  case mvsc::script::kScriptValueString:
    if (lua_type(L, abs_value_index) != LUA_TSTRING) {
  ...
  case mvsc::script::kScriptValueBoolArray: {
    ...
    lua_Integer len = 0;
    if (!getDenseArrayLength(L, abs_value_index, &len)) {
      return luaL_error(L, "SetBoolArrayValue expects a dense array table");
    }
    ...
    for (std::size_t idx = 0; idx < debug_value.bool_array_value.size(); ++idx) {
      pOutFrame->setVal(outputName, static_cast<int>(idx),
                        debug_value.bool_array_value[idx]);
    }
    break;
  }
  case mvsc::script::kScriptValueIntArray: {
    ...
    lua_Integer len = 0;
    if (!getDenseArrayLength(L, abs_value_index, &len)) {
      return luaL_error(L, "SetIntArrayValue expects a dense array table");
    }
    for (lua_Integer idx = 1; idx <= len; ++idx) {
      lua_rawgeti(L, abs_value_index, idx);
    ...
    for (std::size_t idx = 0; idx < debug_value.int_array_value.size(); ++idx) {
      pOutFrame->setVal(outputName, static_cast<int>(idx),
                        debug_value.int_array_value[idx]);
    }
    break;
  }
  case mvsc::script::kScriptValueFloatArray: {
    ...
    lua_Integer len = 0;
    if (!getDenseArrayLength(L, abs_value_index, &len)) {
      return luaL_error(L, "SetFloatArrayValue expects a dense array table");
    }
    for (lua_Integer idx = 1; idx <= len; ++idx) {
      lua_rawgeti(L, abs_value_index, idx);
      if (!lua_isnumber(L, -1)) {
    ...
    for (std::size_t idx = 0; idx < debug_value.float_array_value.size(); ++idx) {
      pOutFrame->setVal(outputName, static_cast<int>(idx),
                        debug_value.float_array_value[idx]);
    }
    break;
  }
  case mvsc::script::kScriptValueStringArray: {
    ...
    lua_Integer len = 0;
    if (!getDenseArrayLength(L, abs_value_index, &len)) {
      return luaL_error(L, "SetStringArrayValue expects a dense array table");
    }
    for (lua_Integer idx = 1; idx <= len; ++idx) {
      lua_rawgeti(L, abs_value_index, idx);
    ...
    for (std::size_t idx = 0; idx < debug_value.string_array_value.size(); ++idx) {
      pOutFrame->setVal(outputName, static_cast<int>(idx),
                        debug_value.string_array_value[idx]);
    }
    break;
  }
  default:

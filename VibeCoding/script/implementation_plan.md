...
- Landed:
  - Chunk 1 parser/analyzer redesign
  - Chunk 2 persistent runtime hosting
  - Chunk 3 typed getter/setter binding and script-driven metadata generation
  - Chunk 4 shipped template/config cleanup
  - shared Host API registry for parser/runtime extension
  - dedicated host-side runtime harness and focused framework metadata tests
- Remaining:
  - add comif/adapter-path regression coverage if that layer will continue to change
  - resolve native `make -C source/algos/modules/script` verification separately from the host harness
...
- [x] Replace old `ScriptVarDef`-centric analysis outputs with explicit metadata structs for:
  - lifecycle presence
  - typed subscriptions
  - typed published outputs
  - declared globals
  - validation issues
- [x] Keep issue reporting line/column capable.
- [x] Reserve enum coverage for scalar and array types.
- [x] Document in comments which metadata is compile-time only vs runtime-consumed.
...
- [x] Add tests for valid `run()` with `GetIntValue()` and `SetIntValue()`.
- [x] Add tests for valid `init()` global declarations and later use in `run()`.
- [x] Add tests for array getter/setter extraction.
- [x] Add tests that reject:
  - missing `run()`
...
- [x] Run only the affected unit tests and confirm they fail for the expected missing-behavior reasons.
...
- [x] Teach the analyzer to detect lifecycle function declarations.
- [x] Teach the analyzer to collect typed getter/setter calls from `run()`.
- [x] Teach the analyzer to collect legal global declarations from `init()`.
- [x] Reject dynamic call arguments that prevent static extraction.
- [x] Reject unsupported globals and type conflicts.
- [x] Keep dangerous-API rejection intact.
- [x] Re-run the targeted parser tests and make them pass.
...
- [x] Remove or deprecate fields dedicated only to `InputList` / `OutputList` / `ctx`.
- [x] Add stored parse results for subscriptions, outputs, and globals.
- [x] Add persistent Lua-state ownership fields.
- [x] Add flags for environment readiness, `init()` success, and teardown status.
- [x] Keep mutex ownership explicit because runtime is no longer per-trigger ephemeral.
...
- [x] If full operator-level automated tests are not yet practical, add documented executable checks for:
  - one-time `init()`
...
- [x] Replace per-trigger `lua_newstate()`/`lua_close()` execution with a persistent activated environment.
...
- [x] Preserve sandbox restrictions and timeout/memory controls.
- [x] Make `Process()` fail fast if activation never completed successfully.
- [x] Ensure script replacement destroys the old environment after optional `cleanup()`.
...
- [x] Add Lua-callable bindings for all scalar getters.
- [x] Add Lua-callable bindings for all array getters, returning Lua tables.
- [x] Add Lua-callable bindings for all scalar output setters.
- [x] Add Lua-callable bindings for all array output setters, validating table shape and element type.
- [x] Reserve but do not yet implement module-parameter API names.
- [x] Emit clear runtime errors when setters receive mismatched value types.
- [x] Centralize host API definitions so parser validation and runtime binding
  share one registry.
- [x] Add `GetGlobalCommunicationData()` as a context getter example that does
  not affect topology metadata.
...
- [x] Build dynamic subscribe metadata directly from parsed getter calls.
- [x] Build dynamic publish metadata directly from parsed setter calls.
- [x] Deduplicate repeated script references at the metadata layer.
- [x] Keep upper-layer topology validation as a separate responsibility.
- [x] Remove old `script_input0..31` assumptions from the script-module control flow.
...
- [x] Run parser tests.
- [x] Run any available script-operator smoke verification.
- [x] Verify through host-side runtime tests:
  - `init()` executes once
...
- [x] Replace the default script template with the new helper-function ABI.
- [x] Remove or retire obsolete manual input subscription parameters from the formal shipped configuration.
- [x] Keep only the config surface that is still required to store script text and compile/check actions.
- [x] Ensure metadata no longer instructs users toward `inputs/outputs` or `script_input0..31`.
...
- [x] Update implementation status after code and tests actually pass.
- [x] Record remaining gaps, especially if module-parameter APIs remain reserved-only.

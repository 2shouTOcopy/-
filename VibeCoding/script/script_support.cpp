...
ScriptValueType inferAssignedType(const std::vector<Token> &tokens, size_t start,
  ...
  const ScriptHostApiDef *api = FindScriptHostApi(token.text);
  if (api != NULL &&
      (api->kind == kScriptHostApiTopologyGetter ||
       api->kind == kScriptHostApiContextGetter) &&
      start + 1 < end &&
      tokens[start + 1].text == "(") {
    return api->value_type;
  }
...
int lifecycleMaskForFunction(FunctionKind function_kind) {
  switch (function_kind) {
  case kFunctionInit:
    return kScriptLifecycleInit;
  case kFunctionRun:
    return kScriptLifecycleRun;
  case kFunctionCleanup:
    return kScriptLifecycleCleanup;
  default:
    return 0;
  }
}

int countCallArguments(const std::vector<Token> &tokens, size_t open_index,
                       size_t close_index) {
  ...
  int depth = 0;
  int count = 1;
  for (size_t idx = open_index + 1; idx < close_index; ++idx) {
    if (tokens[idx].text == "(" || tokens[idx].text == "{" ||
        tokens[idx].text == "[") {
      ++depth;
    } else if (tokens[idx].text == ")" || tokens[idx].text == "}" ||
               tokens[idx].text == "]") {
      --depth;
    } else if (depth == 0 && tokens[idx].text == ",") {
      ++count;
    }
  }
  return count;
...
ScriptAnalysisResult AnalyzeScript(const std::string &script_text,
  ...
  for (std::size_t idx = 0; idx < tokens.size(); ++idx) {
    ...
    const ScriptHostApiDef *host_api = FindScriptHostApi(token.text);
    if (host_api != NULL && idx + 1 < tokens.size() && tokens[idx + 1].text == "(") {
      if (host_api->kind == kScriptHostApiReserved) {
      ...
      const int lifecycle_mask = lifecycleMaskForFunction(current_function);
      if ((host_api->lifecycle_mask & lifecycle_mask) == 0) {
        addIssue(result, kScriptIssueUnsupportedHostApi,
                 "host API is not allowed in this lifecycle function",
                 token.text, token.line, token.column);
        return result;
      }

      const int arg_count = countCallArguments(tokens, idx + 1, close_index);
      if (arg_count < host_api->min_args || arg_count > host_api->max_args) {
        addIssue(result, kScriptIssueUnsupportedHostApi,
                 "host API called with unsupported argument count", token.text,
                 token.line, token.column);
        return result;
      }

      if (current_function == kFunctionRun &&
          host_api->kind == kScriptHostApiTopologyGetter) {
        ...
        if (existing == subscription_index.end()) {
          ...
          return result;
                 ...
                 host_api->kind == kScriptHostApiOutputSetter) {
        ...
        } else if (result.outputs[existing->second].type !=
                   host_api->value_type) {

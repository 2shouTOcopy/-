...
- Covered today by automated tests:
  - parser/analyzer static-rule cases in `ScriptSupportTest.*`
  - persistent `init()` state across triggers
  - string-array getter/setter marshaling
  - bool/int/float array getter/setter marshaling
  - `init()` runtime failure
  - scalar setter type mismatch
  - numeric values rejected by string setters
  - `run()` runtime error reporting
  - timeout and memory-limit error paths
  - context getter registration and runtime binding through
    `GetGlobalCommunicationData()`
  - context getters do not create topology subscription metadata
  - context getter arity validation
  - missing subscribed value reporting
  - sparse int-array rejection
  - non-table array setter rejection
  - mixed-type string-array rejection without partial publication
  - explicit `cleanup()` failure teardown behavior
  - framework name-based/static-vs-dynamic publish metadata behavior
- Still not yet covered by automated tests:
  - comif/adapter publish-source regression coverage
...
- dynamic-publish modules expose subscribe-source outputs from runtime result
  metadata instead of static `io_conf.json`

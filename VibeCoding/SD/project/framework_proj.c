...
int32_t modify_sln_file_info_by_media(StorageMedia media, const char *proj_name,
								  const struct project_info *proj_info,
								  const struct switch_info *swth_info);
int32_t modify_algo_project_info(const char *proj_name, const struct project_info *proj_info,
								 const struct switch_info *swth_info);
int32_t modify_algo_project_info_by_media(StorageMedia media, const char *proj_name,
								 const struct project_info *proj_info,
								 const struct switch_info *swth_info);
int32_t modify_emmc_sln_switch_info(const char *proj_name, const struct switch_info *swth_info);
int32_t modify_sln_switch_info_by_media(StorageMedia media, const char *proj_name,
								 const struct switch_info *swth_info);
...
static int32_t make_parent_dir(const char *file_path);
static int32_t resolve_project_access_paths(StorageMedia media, const char *project_name,
											StorageProjectSavePaths *paths);
...
static int scfw_slnar_header_hook_save_baseimage(SLNAR_T* slnar, SLNAR_DEV_INFO_T* devinfo, SLNAR_SLN_INFO_T* slninfo)
{
	...
	StorageProjectSavePaths storage_paths = {0};
	int ret = storage_project_resolve_save_paths(STORAGE_MEDIA_AUTO, slninfo->baseinfo.name,
		0, &storage_paths);
	if (ret < 0)
	{
		LOGE("[%s] storage_project_resolve_save_paths failed:%d\n", __func__, ret);
		return -2;
	}

	if (make_parent_dir(storage_paths.base_image_path) < 0)
	{
		return -3;
	}

	if (SLNAR_EC_OK != slnar_set_baseimage_path(slnar, storage_paths.base_image_path))
	{
		return -4;
	}
...
{
	int ret = 0;
	char dir_path[STORAGE_MAX_PATH_LEN] = {0};
	...
	if ((NULL == last_slash) || (last_slash == dir_path))
	{
		return 0;
	}

	*last_slash = '\0';
	if (FALSE == osal_is_dir_exist(dir_path))
	{
		ret = osal_create_dir(dir_path);
		if (ret < 0)
		{
...
static int32_t resolve_project_access_paths(StorageMedia media, const char *project_name,
											StorageProjectSavePaths *paths)
{
	int ret = storage_project_resolve_access_paths(media, project_name, paths);
	if (ret < 0)
	{
		LOGE("[%s] storage_project_resolve_access_paths %s failed:%d\n",
			 __func__, project_name ? project_name : "NULL", ret);
	}
	return ret;
}

static int32_t make_recursive_copy_source_arg(const char *path, char *buf, size_t len)
{
	int ret = 0;

	if ((NULL == path) || (NULL == buf) || (0 == len))
	{
		return STORAGE_E_INVALID_PARAM;
	}

	ret = snprintf(buf, len, "%s -r", path);
...
static int32_t save_algo_project_mng_into_path(cJSON *proj_mng, const char *file_path)
{
	int error = 0;
	int fsize = 0;
	int ret = 0;
	...
	if ((NULL == proj_mng) || (NULL == file_path))
	{
		LEAVE(-1, out);
	}
...
{
	char index_path[STORAGE_MAX_PATH_LEN] = {0};
	int ret = storage_project_get_index_path(STORAGE_MEDIA_EMMC,
		index_path, sizeof(index_path));
	if (ret < 0)
	{
		LOGE("[%s] storage_project_get_index_path failed:%d\n", __func__, ret);
		return ret;
	}

	return save_algo_project_mng_into_path(proj_mng, index_path);
}
...
int32_t scfw_set_cur_project_name(const char *project_name)
{
	int error = 0;
	int ret = 0;
	char src_path[STORAGE_MAX_PATH_LEN] = {0};
	char cur_path[STORAGE_MAX_PATH_LEN] = {0};
	...
	{
		ret = storage_project_get_workdir_path(STORAGE_MEDIA_EMMC,
			project_name, src_path, sizeof(src_path));
		if (ret < 0)
		{
			LOGE("storage_project_get_workdir_path %s error ret = %d\n",
				project_name, ret);
			LEAVE(-2, out);
		}
		ret = storage_project_get_workdir_path(STORAGE_MEDIA_EMMC,
			cur_project_name, cur_path, sizeof(cur_path));
		if (ret < 0)
		{
			LOGE("storage_project_get_workdir_path %s error ret = %d\n",
				cur_project_name, ret);
			LEAVE(-2, out);
		}
		...
		{
			ret = storage_project_get_workdir_path(STORAGE_MEDIA_EMMC,
				DEFAULT_PROJ_NAME, src_path, sizeof(src_path));
			if (ret < 0)
			{
				LOGE("storage_project_get_workdir_path %s error ret = %d\n",
					DEFAULT_PROJ_NAME, ret);
				LEAVE(-3, out);
			}
...
int32_t scfw_get_cur_project_path(char *path, int len)
{
	...
	ret = storage_project_get_workdir_dir_path(STORAGE_MEDIA_EMMC,
		scfw_get_cur_project_name(), dir_path, sizeof(dir_path));
	if (ret < 0)
	{
		return ret;
	}

	if ((int)strlen(dir_path) >= len)
	{
		return -2;
	}
...
{
	int ret = 0;
	char fpath[STORAGE_MAX_PATH_LEN] = {0};
	char project_work_root[STORAGE_MAX_PATH_LEN] = {0};
	...
	ret = storage_project_get_workdir_root_dir_path(STORAGE_MEDIA_EMMC,
		project_work_root, sizeof(project_work_root));
	if (ret < 0)
	{
		LOGE("storage_project_get_workdir_root_dir_path error:%d\n", ret);
		return ret;
	}

	if ((ret = osal_get_dir_num(project_work_root, 0, &dir_num)) < 0)
	{
	...
	{
		if ((ret = osal_opendir(&dir, project_work_root)) < 0)
		{
			printf("osal_opendir ret = %d\n", ret);
			return -1;
		}

		while (OSAL_NOT_EXIST != osal_readdir(dir, project_work_root, &dir_info))
		{
				...
				&& ((strncmp(dir_info.name, DEFAULT_PROJ_NAME, MAX_FNAME_LEN))))
			{
				LOGI("file_name:%s\n", dir_info.name);
				ret = storage_project_get_workdir_path(STORAGE_MEDIA_EMMC,
					dir_info.name, fpath, sizeof(fpath));
				if (ret < 0)
				{
					LOGE("storage_project_get_workdir_path %s error:%d\n",
						dir_info.name, ret);
					break;
				}
...
int32_t scfw_get_project_path_ddr(char *path, const char *project_name, int len)
{
	...
	if ((NULL == path) || (NULL == project_name) || (len <= 0))
	{
		return -1;
	}

	ret = storage_project_get_workdir_dir_path(STORAGE_MEDIA_EMMC,
		project_name, dir_path, sizeof(dir_path));
	if (ret < 0)
	{
		return ret;
	}
...
static int32_t load_algo_project_mng_from_path(cJSON **algo_proj_mng, const char *file_path)
{
	int error = 0;
	int fsize = 0;
	int32_t gsize = 0;
	...
	if ((NULL == algo_proj_mng) || (NULL == file_path))
	{
		LEAVE(-1, out);
	}
...
static int32_t load_algo_project_mng_from_media(StorageMedia media, cJSON **algo_proj_mng)
{
	char file_path[STORAGE_MAX_PATH_LEN] = {0};
	int ret = 0;
	...
	ret = storage_project_get_index_path(media, file_path, sizeof(file_path));
	if (ret < 0)
	{
		LOGE("[%s] storage_project_get_index_path failed:%d\n", __func__, ret);
		return -2;
	...
	return load_algo_project_mng_from_path(algo_proj_mng, file_path);
}

static int32_t load_algo_project_mng_from_file(cJSON **algo_proj_mng)
{
	return load_algo_project_mng_from_media(STORAGE_MEDIA_EMMC, algo_proj_mng);
}
...
static int32_t scfw_collect_sln_files(struct sln_file_entry *entries, uint32_t *entry_num)
{
	...
	void *dir = NULL;
	struct osal_dir_info dir_info = {0};
	char package_root[STORAGE_MAX_PATH_LEN] = {0};
	int ret = 0;
	...
	*entry_num = 0;
	ret = storage_project_get_package_root_dir_path(STORAGE_MEDIA_EMMC,
		package_root, sizeof(package_root));
	if (ret < 0)
	{
		LOGE("[%s] storage_project_get_package_root_dir_path failed:%d\n",
			__func__, ret);
		LEAVE(-2, out);
	}

	if (osal_opendir(&dir, package_root) < 0)
	{
		LEAVE(-3, out);
	}

	while (OSAL_NOT_EXIST != osal_readdir(dir, package_root, &dir_info))
	{
...
static int32_t scfw_sync_project_mng_with_sln_files(void)
{
	...
	if ((ret = storage_project_get_package_root_dir_path(STORAGE_MEDIA_EMMC,
		package_root, sizeof(package_root))) < 0)
	{
		LOGE("[%s] storage_project_get_package_root_dir_path failed:%d\n",
			__func__, ret);
		LEAVE(-1, out);
	...
	if ((ret = storage_project_get_index_path(STORAGE_MEDIA_EMMC,
		index_path, sizeof(index_path))) < 0)
	{
		LOGE("[%s] storage_project_get_index_path failed:%d\n", __func__, ret);
		LEAVE(-1, out);
	}

	if (osal_is_dir_exist(package_root))
	{
	...
	if (osal_is_file_exist(index_path))
	{
	...
	for (uint32_t i = 0; i < entry_num; i++)
	{
		...
		ret = snprintf(file_path, sizeof(file_path), "%s%s",
			package_root, entries[i].filename);
		if (ret < 0 || (size_t)ret >= sizeof(file_path))
		{
			LOGE("[%s] project package path too long:%s\n",
				__func__, entries[i].filename);
			continue;
		}
...
static int delete_saved_project_by_media(StorageMedia media, const char *project_name, const char *passwd)
{
	int ret = 0;
	char file_path[MAX_FILE_NAME_LEN * 2] = {0};
	StorageProjectSavePaths storage_paths = {0};
	cJSON *project_mng_root = NULL;
	cJSON *proj_obj = NULL;
	...
	if (resolve_project_access_paths(media, project_name, &storage_paths) < 0)
	{
		return -1;
	}

	if (storage_paths.media == STORAGE_MEDIA_MICROSD)
	{
		use_temp_root = 1;
		if (osal_is_file_exist(storage_paths.index_path) &&
			(load_algo_project_mng_from_media(storage_paths.media, &project_mng_root) < 0))
		{
			return -2;
		}
	}
	else
	{
		...
		if ((NULL == algo_proj_mng_root) && (osal_is_file_exist(storage_paths.index_path)))
		{
		...
		project_mng_root = algo_proj_mng_root;
	}

	if ((NULL != project_mng_root) && ((proj_obj = get_project_obj(project_mng_root, project_name)) != NULL))
	{
		if (passwd)
		{
		...
		if ((tmp_obj = cJSON_GetObjectItem(project_mng_root, JSON_PROJECT_NUM)) == NULL)
		{
			return -5;
		}
		...
		{
			if ((ret = delete_project_obj(project_mng_root, project_name)) < 0)
			{
				return -6;
			}
			...
			/* write the json str into file */
			if (save_algo_project_mng_into_path(project_mng_root, storage_paths.index_path) < 0)
			{
				return -7;
			}

			if ((storage_paths.media == STORAGE_MEDIA_EMMC) && (update_project_switch_mng() < 0))
			{
				return -8;
			}
		}
		...
		{
			if (osal_remove_file(storage_paths.index_path) < 0)
			{
				LOGE("[%s] osal_remove_file:%s err !\n", __func__, storage_paths.index_path);
			}
			if (use_temp_root)
			{
				cJSON_Delete(project_mng_root);
				project_mng_root = NULL;
			}
			else
			{
				cJSON_Delete(algo_proj_mng_root);
	...
	snprintf(file_path, sizeof(file_path), "%s", storage_paths.package_path);
	if (osal_remove_file(file_path) < 0)
	{
		LOGE("osal_remove_dir fail:ret %s\r\n", file_path);
	}

	if (use_temp_root && project_mng_root)
	{
		cJSON_Delete(project_mng_root);
	}
...
{
	return delete_saved_project_by_media(STORAGE_MEDIA_AUTO, project_name, passwd);
}
...
int32_t scfw_copy_project_to_ddr(const char *dst_proj_name, const char *passwd, const char *src_proj_name)
{
	int32_t ret = 0;
	int32_t error = 0;
	char dst_path[STORAGE_MAX_PATH_LEN] = {0}; /*目标路径*/
	char src_path[STORAGE_MAX_PATH_LEN] = {0}; /*源路径*/
	char base_image_path[STORAGE_MAX_PATH_LEN] = {0};
	StorageProjectSavePaths src_storage_paths = {0};
	StorageProjectSavePaths dst_storage_paths = {0};
	...
	ret = storage_project_resolve_copy_paths(STORAGE_MEDIA_AUTO, src_proj_name,
		dst_proj_name, MAX_UPDATE_SLN_LEN, &src_storage_paths, &dst_storage_paths);
	if (ret < 0)
	{
		LOGE("[%s] storage_project_resolve_copy_paths failed:%d\n", __func__, ret);
		LEAVE(ret, out);
	}

	snprintf(src_path, sizeof(src_path), "%s", src_storage_paths.package_path);
	...
	snprintf(dst_path, sizeof(dst_path), "%s", dst_storage_paths.package_path);
    ...
    snprintf(base_image_path, sizeof(base_image_path),
        "%s", dst_storage_paths.base_image_path);
...
int32_t scfw_save_as_project_to_ddr(const char *project_name, const char *passwd, const char *src_proj_name)
{
	...
	int32_t error = 0;
	int32_t language = 0;
	char file_path[STORAGE_MAX_PATH_LEN] = {0};/*方案文件路径 */
	char dst_path[STORAGE_MAX_PATH_LEN] = {0}; /*目标路径*/
	char src_path[STORAGE_MAX_PATH_LEN] = {0}; /*目标路径*/
	char base_image_path[STORAGE_MAX_PATH_LEN] = {0}; /*基准图文件路径*/
    int base_image_exist = 1;
	char cur_proj_path[STORAGE_MAX_PATH_LEN + 4] = {0};
	StorageProjectSavePaths storage_paths = {0};
	struct project_info proj_info = {0};
	struct switch_info swth_info = {0};
	...
	ret = storage_project_resolve_save_paths(STORAGE_MEDIA_AUTO, project_name,
		MAX_UPDATE_SLN_LEN, &storage_paths);
	if (ret < 0)
	{
		LOGE("[%s] storage_project_resolve_save_paths failed:%d\n", __func__, ret);
		LEAVE(ret, out);
	...
	if ((ret = make_parent_dir(storage_paths.package_path)) < 0)
	{
		LEAVE(ret, out);
	}

	if ((ret = make_parent_dir(storage_paths.base_image_path)) < 0)
	{
		LEAVE(ret, out);
	}
	...
	if (strncmp(cur_project_name, project_name, MAX_FNAME_LEN))
	{
		...
		if (ret < 0)
		{
			LOGE("[%s] storage_project_get_workdir_path %s error:%d\n",
				__func__, project_name, ret);
			LEAVE(ret, out);
		}

		ret = storage_project_get_workdir_path(STORAGE_MEDIA_EMMC,
			src_proj_name, cur_proj_path, sizeof(cur_proj_path));
		if (ret < 0)
		{
			LOGE("[%s] storage_project_get_workdir_path %s error:%d\n",
				__func__, src_proj_name, ret);
			LEAVE(ret, out);
		}
	...
	if (base_image_exist)
	{
		snprintf(file_path, sizeof(file_path), "%s", storage_paths.base_image_path);
	...
	/* 写入数据到文件中 */
	if ((ret = save_algo_project_mng_into_path(algo_proj_mng_root,
		storage_paths.index_path)) < 0)
	{
		LOGE("[%s] save_algo_project_mng_into_file:%d\n", __func__, ret);
		LEAVE(-18, out);
	}
...
out:
	if (error)
	{
		...
		{
			ret = storage_project_get_workdir_path(STORAGE_MEDIA_EMMC,
				src_proj_name, src_path, sizeof(src_path));
			if (ret < 0)
			{
				LOGE("[%s] storage_project_get_workdir_path %s error:%d\n",
					__func__, src_proj_name, ret);
			...
			ret = storage_project_get_workdir_path(STORAGE_MEDIA_EMMC,
				project_name, dst_path, sizeof(dst_path));
			if (ret < 0)
			{
				LOGE("[%s] storage_project_get_workdir_path %s error:%d\n",
					__func__, project_name, ret);
		...
		if (error < -4) /*其他的需要恢复目录名称要不一样*/
		{
			if (strncmp(cur_project_name, src_proj_name, MAX_FNAME_LEN)) /*名字不一样*/
			{
				...
				ret = make_recursive_copy_source_arg(src_path, cur_proj_path,
					sizeof(cur_proj_path));
				if (ret < 0)
				{
					LOGE("[%s] make_recursive_copy_source_arg %s error:%d\n",
...
int32_t scfw_save_project_to_path(const char *project_name, const char *passwd, const char *save_path)
{
	...
	if (storage_project_is_emmc_package_root_path(save_path))
	{
		ret = storage_project_resolve_save_paths(STORAGE_MEDIA_AUTO, project_name,
			MAX_UPDATE_SLN_LEN, &storage_paths);
		if (ret < 0)
	...
	else if (FALSE == osal_is_dir_exist(save_path)) /*目录不存在创建目录*/
	{
	...
	else if (FALSE == osal_is_dir_exist(DEVSLN_BASE_IMG_DIR)) /*目录不存在创建目录*/
	{
		LOGI("The save_path %s is not exist.Now creat path!!\n", DEVSLN_BASE_IMG_DIR);
	...
	if (strncmp(cur_project_name, project_name, MAX_FNAME_LEN))
	{
		...
		ret = storage_project_get_workdir_path(STORAGE_MEDIA_EMMC,
			cur_project_name, cur_proj_path, sizeof(cur_proj_path));
		if (ret < 0)
		{
			LOGE("[%s] storage_project_get_workdir_path %s error:%d\n",
				__func__, cur_project_name, ret);
			LEAVE(ret, out);
		}
		...
		{
			ret = storage_project_get_workdir_path(STORAGE_MEDIA_EMMC,
				DEFAULT_PROJ_NAME, dst_path, sizeof(dst_path));
			if (ret < 0)
			{
				LOGE("[%s] storage_project_get_workdir_path %s error:%d\n",
					__func__, DEFAULT_PROJ_NAME, ret);
				LEAVE(ret, out);
			}

			if ((ret = osal_create_dir(dst_path)) < 0)
			{
				LOGE("osal copy %s %s error\n", cur_proj_path, dst_path);
				LEAVE(-4, out);
			}
	...
	{
		ret = storage_project_get_external_save_file_path(save_path, project_name,
			file_path, sizeof(file_path));
		if (ret < 0)
		{
			LOGE("[%s] storage_project_get_external_save_file_path failed:%d\n",
				__func__, ret);
			LEAVE(ret, out);
		}
	}
	...
	if (storage_project_is_emmc_package_root_path(save_path))
	{
		...
		if (use_storage_paths)
		{
			ret = save_algo_project_mng_into_path(algo_proj_mng_root,
				storage_paths.index_path);
		}
		else
		{
			ret = save_algo_project_mng_into_file(algo_proj_mng_root);
		}
		if (ret < 0)
		{
		...
		*/
		...
		if (error < -4) /*其他的需要恢复目录名称要不一样*/
		{
			...
			if (strncmp(cur_project_name, pre_project_name, MAX_FNAME_LEN)) /*名字不一样*/
			{
				...
				{
					ret = storage_project_get_workdir_path(STORAGE_MEDIA_EMMC,
						pre_project_name, dst_path, sizeof(dst_path));
					if (ret < 0)
					{
						LOGE("[%s] storage_project_get_workdir_path %s error:%d\n",
							__func__, pre_project_name, ret);
						return error;
					}
				...
				ret = storage_project_get_workdir_path(STORAGE_MEDIA_EMMC,
					pre_project_name, src_path, sizeof(src_path));
				if (ret < 0)
				{
					LOGE("[%s] storage_project_get_workdir_path %s error:%d\n",
						__func__, pre_project_name, ret);
								   ...
								   uint32_t type,bool need_push_rate,bool need_push_fail)
{
	...
	char base_image_path[STORAGE_MAX_PATH_LEN] = {0};
	char file_path[STORAGE_MAX_PATH_LEN] = {0};
	char dst_path[STORAGE_MAX_PATH_LEN] = {0};
	StorageProjectSavePaths storage_paths = {0};
	cJSON *proj_obj = NULL;
	cJSON *passwd_obj = NULL;
	...
	if (resolve_project_access_paths(media, proj_name, &storage_paths) < 0)
	{
		LEAVE(-1, out);
	}
	paths_resolved = 1;
	...
	if (storage_paths.media == STORAGE_MEDIA_MICROSD)
	{
		if (algo_proj_mng_root)
		{
			cJSON_Delete(algo_proj_mng_root);
			algo_proj_mng_root = NULL;
		}
		if (load_algo_project_mng_from_media(storage_paths.media, &algo_proj_mng_root) < 0)
		{
			LEAVE(-1, out);
		}
	}
	else if (NULL == algo_proj_mng_root)
	{
		if (load_algo_project_mng_from_file(&algo_proj_mng_root) < 0)
		{
			LEAVE(-1, out);
	...
	{
		snprintf(file_path, sizeof(file_path), "%s", storage_paths.workdir_path);
	...
	if (!osal_is_dir_exist(file_path))
	{
		...
		snprintf(file_path, sizeof(file_path), "%s", storage_paths.package_path);
        snprintf(dst_path, sizeof(dst_path), "%s", storage_paths.workdir_path);
		snprintf(base_image_path, sizeof(base_image_path), "%s", storage_paths.base_image_path);
...
out:
	...
	{
		if ((0 != error) && paths_resolved)
		{
			ret = storage_project_get_workdir_path(storage_paths.media,
				proj_name, file_path, sizeof(file_path));
			if (ret < 0)
			{
				LOGE("[%s] storage_project_get_workdir_path %s error:%d\n",
					__func__, proj_name, ret);
			}
			else
			{
		...
		if (paths_resolved)
		{
			do
			{
				...
				ret = storage_project_get_package_path(storage_paths.media,
					proj_name, file_path, sizeof(file_path));
				if (ret < 0)
				{
					LOGE("[%s] storage_project_get_package_path %s error:%d\n",
						__func__, proj_name, ret);
					break;
				}
...
{
	return scfw_load_project_by_media(STORAGE_MEDIA_AUTO, proj_name, passwd,
		type, need_push_rate, need_push_fail);
}
...
int32_t scfw_load_default_project_with_PR(bool pr_flag)
{
	...
	struct module_name_info name_info = {0};
	char solution_dir[STORAGE_MAX_PATH_LEN] = {0};
	char default_project_dir[STORAGE_MAX_PATH_LEN] = {0};
	char dir1[STORAGE_MAX_PATH_LEN] = {0};
	char dir2[STORAGE_MAX_PATH_LEN] = {0};
	...
	{
		ret = storage_project_get_workdir_path(STORAGE_MEDIA_EMMC,
			current_solution_name, solution_dir, sizeof(solution_dir));
		if (ret < 0)
		{
			LEAVE(ret, out);
		}
	...
	ret = storage_project_get_workdir_path(STORAGE_MEDIA_EMMC,
		DEFAULT_PROJ_NAME, default_project_dir, sizeof(default_project_dir));
	if (ret < 0)
	{
		LEAVE(ret, out);
	}

	if ((ret = osal_remove_dir(default_project_dir)) != 0)
	{
		LOGW("[%s]osal_create_dir default error:%d\n", __func__, ret);
	}

	if ((ret = osal_create_dir(default_project_dir)) != 0)
	{
		LOGW("[%s]osal_create_dir default error:%d\n", __func__, ret);
	}
	...
	ret = storage_project_get_workdir_child_path(STORAGE_MEDIA_EMMC,
		DEFAULT_PROJ_NAME, "vtool", dir1, sizeof(dir1));
	if (ret < 0)
	{
		LEAVE(ret, out);
	}
	ret = storage_project_get_workdir_child_path(STORAGE_MEDIA_EMMC,
		DEFAULT_PROJ_NAME, "ctool", dir2, sizeof(dir2));
	if (ret < 0)
	{
		LEAVE(ret, out);
	}
...
int32_t scfw_load_default_project(void)
{
	...
	if (true == osal_is_dir_exist(default_project_dir))
	{
		if ((ret = osal_remove_dir(default_project_dir)) != 0)
		{
...
int32_t scfw_delete_project_by_media(StorageMedia media, const char *project_name)
{
	...
	{
		use_temp_root = 1;
		if (load_algo_project_mng_from_media(storage_paths.media, &project_mng_root) < 0)
		{
			LEAVE(-2, out);
		}
	}
	...
	if ((proj_obj = get_project_obj(project_mng_root, project_name)) != NULL)
	{
		if ((tmp_obj = cJSON_GetObjectItem(project_mng_root, JSON_PROJECT_NUM)) == NULL)
		{
			LEAVE(-3, out);
...
{
	return scfw_delete_project_by_media(STORAGE_MEDIA_AUTO, project_name);
}
/**
...
{
	char package_root[STORAGE_MAX_PATH_LEN] = {0};
	int ret = storage_project_get_package_root_dir_path(STORAGE_MEDIA_EMMC,
		package_root, sizeof(package_root));
	if (ret < 0)
	{
		LOGE("[%s] storage_project_get_package_root_dir_path failed:%d\n",
	...
	if (osal_remove_dir(package_root))
	{
		LOGE("free_algo_project();from mng.c hjj\n");
		return -1;
	}

	if (osal_create_dir(package_root))
	{
		LOGE("free_algo_project();from mng.c hjj\n");
		return -1;
	}
...
int32_t scfw_upload_project(const char *proj_file_path)
{
	...
	project_file_size = osal_get_file_size(proj_file_path);
	if (project_file_size < 0)
	{
		LOGE("[%s] osal_get_file_size failed:%d, path:%s\n",
			__func__, project_file_size, proj_file_path);
		LEAVE(-10, out);
	}

	ret = storage_project_resolve_save_paths(STORAGE_MEDIA_AUTO, baseinfo.name,
		(uint64_t)project_file_size, &storage_paths);
	if (ret < 0)
	{
		LOGE("[%s] storage_project_resolve_save_paths failed:%d\n", __func__, ret);
		LEAVE(ret, out);
	...
	/* write the json str into file */
	if (save_algo_project_mng_into_path(algo_proj_mng_root,
		storage_paths.index_path) < 0)
	{
		LEAVE(-12, out);
	}
...
/**
  ...
  * @return  0 on success; < 0 on failure
  */
static int32_t scfw_find_project_info_by_media(StorageMedia media, const char *proj_name,
											   struct project_info *proj_info)
{
	uint32_t project_num = 0;
	uint32_t i = 0;
	...
	if (scfw_get_project_num_by_media(media, &project_num) < 0)
	{
		return -2;
	}
	...
	for (i = 0; i < project_num; i++)
	{
		if (scfw_get_project_info_by_media(media, i, proj_info) < 0)
		{
			continue;
		}
...
int32_t scfw_modify_project_name_by_media(StorageMedia media, const char *old_proj_name, const char *new_proj_name)
{
	int32_t error = 0;
	...
	if (0 == scfw_find_project_info_by_media(media, new_proj_name, &sln_info))
	{
		LEAVE(-3, out);
	}

	memset(&sln_info, 0, sizeof(sln_info));
	if (scfw_find_project_info_by_media(media, old_proj_name, &sln_info) < 0)
	{
		LEAVE(-4, out);
	}
	...
	if (modify_sln_file_info_by_media(media, old_proj_name, &sln_info, NULL))
	{
		LEAVE(-8, out);
	}
...
{
	return scfw_modify_project_name_by_media(STORAGE_MEDIA_AUTO, old_proj_name, new_proj_name);
}
											  ...
											  struct switch_info *swt_info)
{
	...
	if (media == STORAGE_MEDIA_AUTO || media == STORAGE_MEDIA_EMMC)
	{
		return scfw_get_project_switch_info(proj_name, swt_info);
	}
	...
	ret = load_algo_project_mng_from_media(media, &media_root);
	if (ret < 0)
	{
		return ret;
	}

	ret = scfw_get_project_switch_info_from_root(media_root, proj_name, swt_info);
...
static int32_t scfw_get_project_num_from_root(cJSON *project_mng, uint32_t *proj_num)
{
	cJSON *ele_obj = NULL;

	if ((NULL == project_mng) || (NULL == proj_num))
	{
		return -1;
	}

	if ((ele_obj = cJSON_GetObjectItem(project_mng, JSON_PROJECT_NUM)) == NULL)
	{
		*proj_num = 0;
		return -2;
	}
...
static int32_t scfw_get_project_info_from_root(cJSON *project_mng, int proj_idx,
											   struct project_info *proj_info)
{
	cJSON *proj_obj = NULL;
	cJSON *tmp_obj = NULL;

	if ((NULL == project_mng) || (NULL == proj_info))
	{
		return -1;
	}

	proj_obj = get_project_index(project_mng, proj_idx);
	if (NULL == proj_obj)
	{
		return -2;
	}
	...
	tmp_obj = cJSON_GetObjectItem(proj_obj, JSON_PROJECT_IMAGE_SOURCE);
	if ((tmp_obj != NULL) && (tmp_obj->valuestring != NULL))
	{
		strncpy(proj_info->image_source, tmp_obj->valuestring,
			sizeof(proj_info->image_source));
		proj_info->image_source[sizeof(proj_info->image_source) - 1] = '\0';
	}
	else
	{
		strncpy(proj_info->image_source, DEFAULT_IMAGE_SOURCE,
			sizeof(proj_info->image_source));
		proj_info->image_source[sizeof(proj_info->image_source) - 1] = '\0';
	}
...
static void scfw_copy_json_string(cJSON *root, const char *key, char *buf, size_t len)
{
	cJSON *obj = NULL;

	if ((NULL == root) || (NULL == key) || (NULL == buf) || (0 == len))
	{
		return;
	}
	...
	{
		strncpy(buf, obj->valuestring, len);
		buf[len - 1] = '\0';
	}
}

static void scfw_copy_json_int(cJSON *root, const char *key, uint32_t *value)
{
	cJSON *obj = NULL;
	...
	if (obj != NULL)
	{
		*value = (uint32_t)obj->valueint;
	}
}

static void scfw_copy_json_int32(cJSON *root, const char *key, int32_t *value)
{
	cJSON *obj = NULL;
...
static int32_t scfw_get_project_switch_info_from_root(cJSON *project_mng,
													  const char *proj_name,
													  struct switch_info *swt_info)
{
	cJSON *proj_obj = NULL;

	if ((NULL == project_mng) || (NULL == proj_name) || (NULL == swt_info))
	{
		return -1;
	}

	proj_obj = get_project_obj(project_mng, proj_name);
	if (NULL == proj_obj)
	{
		return -2;
	}

	scfw_fill_default_switch_info(swt_info);
	scfw_copy_json_string(proj_obj, JSON_PROJECT_SWITCH_STR,
		swt_info->communication.cm_str, sizeof(swt_info->communication.cm_str));
	scfw_copy_json_string(proj_obj, JSON_PROJECT_SWITCH_SUFFIX_STR,
		swt_info->communication.cm_suffix_str,
		sizeof(swt_info->communication.cm_suffix_str));
	scfw_copy_json_string(proj_obj, JSON_PROJECT_SWITCH_RET,
		swt_info->communication.cm_ret, sizeof(swt_info->communication.cm_ret));
	scfw_copy_json_string(proj_obj, JSON_PROJECT_SWITCH_FAIL,
		swt_info->communication.cm_fail, sizeof(swt_info->communication.cm_fail));
	scfw_copy_json_string(proj_obj, JSON_PROJECT_DI_LINE_CFG,
		swt_info->digital_io.line_cfg, sizeof(swt_info->digital_io.line_cfg));
	scfw_copy_json_string(proj_obj, JSON_PROJECT_TRI_COMM,
		swt_info->communication.trigger_cm_str,
		sizeof(swt_info->communication.trigger_cm_str));
	scfw_copy_json_int(proj_obj, JSON_PROJECT_SWITCH_SOURCE,
		&swt_info->digital_io.source);
	scfw_copy_json_int(proj_obj, JSON_PROJECT_SWITCH_ACTIVE,
		&swt_info->digital_io.active);
	scfw_copy_json_int(proj_obj, JSON_PROJECT_FALSE_TIME,
		&swt_info->digital_io.time);
	scfw_copy_json_int(proj_obj, JSON_PROJECT_DI_ENABLE,
		&swt_info->digital_io.di_enable);
	scfw_copy_json_int(proj_obj, JSON_PROJECT_CM_ENABLE,
		&swt_info->communication.cm_enable);
	scfw_copy_json_int(proj_obj, JSON_PROJECT_IO_INPUT,
		&swt_info->digital_io.io_input);
	scfw_copy_json_int32(proj_obj, JSON_PROJECT_CARM_TRI_SOURCE,
		&swt_info->digital_io.camera_trigger_source);
	scfw_copy_json_int32(proj_obj, JSON_PROJECT_CARM_TRI_MODE,
		&swt_info->digital_io.camera_trigger_mode);
...
int32_t scfw_get_project_num(uint32_t *proj_num)
{
	...
	if (scfw_get_project_num_from_root(algo_proj_mng_root, proj_num) < 0)
	{
		LEAVE(-3, out);
	}
...
int32_t scfw_get_project_num_by_media(StorageMedia media, uint32_t *proj_num)
{
	...
	if (media == STORAGE_MEDIA_AUTO || media == STORAGE_MEDIA_EMMC)
	{
		return scfw_get_project_num(proj_num);
	}
	...
	ret = scfw_get_project_num_from_root(media_root, proj_num);
	cJSON_Delete(media_root);
	return ret;
}
...
bool scfw_check_is_solution_file_exist(const char *name)
{
	...
	ret = storage_project_get_index_path(STORAGE_MEDIA_EMMC,
		index_path, sizeof(index_path));
	if (ret < 0)
	{
		LOGE("[%s] storage_project_get_index_path failed:%d\n", __func__, ret);
...
int32_t scfw_get_project_info(int proj_idx, struct project_info *proj_info)
{
	...
	if (scfw_get_project_info_from_root(algo_proj_mng_root, proj_idx, proj_info) < 0)
	{
		LEAVE(-3, out);
	}
									   ...
									   struct project_info *proj_info)
{
	...
	if (media == STORAGE_MEDIA_AUTO || media == STORAGE_MEDIA_EMMC)
	{
		return scfw_get_project_info(proj_idx, proj_info);
	}
	...
	ret = scfw_get_project_info_from_root(media_root, proj_idx, proj_info);
	cJSON_Delete(media_root);
	return ret;
}
...
int32_t modify_emmc_sln_switch_info(const char *proj_name, const struct switch_info *swth_info)
{
	return modify_sln_switch_info_by_media(STORAGE_MEDIA_AUTO, proj_name, swth_info);
}
								  ...
								  const struct switch_info *swth_info)
{
	...
	*/
	...
	char dst_path[STORAGE_MAX_PATH_LEN] = {0};
	char need_delete = 0;    
	StorageProjectSavePaths old_paths = {0};
	StorageProjectSavePaths new_paths = {0};
    SLNAR_SLN_BASE_INFO_T baseinfo = {0};
    SLNAR_SLN_SW_INFO_T swinfo = {0};
	int base_image_exist = -1;
	...
	if (storage_project_resolve_rename_paths(media, proj_name,
											 proj_info->name, 0,
											 &old_paths, &new_paths) < 0)
	{
		LEAVE(-2, out);
	}

	/*检查方案是否存在osal */
	snprintf(src_path, sizeof(src_path), "%s", old_paths.package_path);
	...
	snprintf(dst_path, sizeof(dst_path), "%s", new_paths.package_path);
	...
	{
		snprintf(src_path, sizeof(src_path), "%s", old_paths.workdir_path);
		snprintf(dst_path, sizeof(dst_path), "%s", new_paths.workdir_path);
		...
		snprintf(src_path, sizeof(src_path), "%s", old_paths.base_image_path);
		snprintf(dst_path, sizeof(dst_path), "%s", new_paths.base_image_path);
	...
	if (modify_algo_project_info_by_media(old_paths.media, proj_name, proj_info, swth_info))
	{
		LEAVE(-11, out);
	}

	if ((old_paths.media == STORAGE_MEDIA_EMMC) && (update_project_switch_mng() < 0))
	{
		LEAVE(-12, out);
	}

	if ((old_paths.media == STORAGE_MEDIA_EMMC) &&
		(ret = update_last_used_project_name(proj_info->name)) < 0)
	{
		LOGE("update_last_used_project_name ret = %d\n", ret);
	}
...
out:	
	...
	if (error < 0)
	{
		...
		if (new_paths.base_image_path[0] != '\0')
		{
			snprintf(dst_path, sizeof(dst_path), "%s", new_paths.base_image_path);
			if ((ret = osal_remove_file(dst_path)) < 0)
			{
...
int32_t modify_emmc_sln_file_info(const char *proj_name, const struct project_info *proj_info,
								  const struct switch_info *swth_info)
{
	return modify_sln_file_info_by_media(STORAGE_MEDIA_AUTO, proj_name, proj_info, swth_info);
}
/**
								 ...
								 const struct switch_info *swth_info)
{
	...
	char index_path[STORAGE_MAX_PATH_LEN] = {0};
	int use_temp_root = 0;
	StorageMedia index_media = (media == STORAGE_MEDIA_AUTO) ? STORAGE_MEDIA_EMMC : media;
	...
	ret = storage_project_get_index_path(index_media, index_path, sizeof(index_path));
	if (ret < 0)
	{
		LEAVE(-2, out);
	}

	if (index_media == STORAGE_MEDIA_MICROSD)
	{
		use_temp_root = 1;
		if (load_algo_project_mng_from_media(index_media, &project_mng_root) < 0)
		{
			LEAVE(-2, out);
		}
	}
	...
	/* replace the valuestring of node "passwd" if chaneged */
	if (((proj_obj = cJSON_GetObjectItem(project_mng_root, JSON_PROJECT_LAST_USED_PROJ)) == NULL)
	...
	{
		cJSON_ReplaceItemInObject(project_mng_root, JSON_PROJECT_LAST_USED_PROJ, cJSON_CreateString((char *)proj_info->name));
	}

	/* is the project already exist ? */
	if ((proj_obj = get_project_obj(project_mng_root, proj_name)) == NULL)
	{
		LEAVE(-4, out);
	}
	else
	{
		...
		/* write the json str into file */
		if (save_algo_project_mng_into_path(project_mng_root, index_path) < 0)
		{
			LEAVE(-20, out);
		}
	}
...
int modify_algo_project_info(const char *proj_name,  const struct project_info *proj_info,
							 const struct switch_info *swth_info)
{
	return modify_algo_project_info_by_media(STORAGE_MEDIA_AUTO, proj_name, proj_info, swth_info);
}
/**
			...
			*/
			...
			if (0 == ret)
			{
				...
				if(0 != strcmp(tmp_project_name, proj_name))
				{
					char file_path[STORAGE_MAX_PATH_LEN] = {0};
					ret = storage_project_get_workdir_path(STORAGE_MEDIA_EMMC,
						tmp_project_name, file_path, sizeof(file_path));
					if (ret < 0)
					{
						LOGE("storage_project_get_workdir_path %s error:%d\n",

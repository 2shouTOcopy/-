...
class CollectdDaemon {
public:
    explicit CollectdDaemon(bool daemonize) : daemonize_(daemonize) {}
...
private:
    ...
    // 写入 PID 文件
    void write_pid_file() {
        std::ofstream pid_file("/var/run/collectd.pid");
        if (pid_file.is_open()) {
    ...
    static void remove_pid_file() {
        unlink("/var/run/collectd.pid");
    }

    // 初始化
    bool initialize() {
        std::cout << "Initializing Collectd..." << std::endl;
        return true;  // 这里可以添加插件加载、配置解析等逻辑
    }

    // 进入主循环
    void main_loop() {
        std::cout << "Collectd is running..." << std::endl;
        while (true) {
...
int main(int argc, char *argv[]) {
    bool daemonize = true;
    CollectdDaemon daemon(daemonize);
    daemon.run();
    return 0;
}